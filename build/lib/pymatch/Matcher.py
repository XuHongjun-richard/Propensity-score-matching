...
class Matcher:
    ...
    def __init__(self, test, control, yvar, formula=None, exclude=[], show_col = 'All'):
        ...
        self.minority, self.majority = [i[1] for i in sorted(zip([self.testn, self.controln],
                                                                 [1, 0]),
                                                             key=lambda x: x[0])]
        if self.show_col == 'All':
            self.show_col = [i for i in self.data.columns if i not in self.exclude and i != yvar]
    ...
    def compare_continuous(self, save=False, return_table=False):
        ...
        for col in self.show_col:
            ...
            if is_continuous and (not (col in self.exclude)):
                ...
                if (np.std(trb.append(cob)) != 0.0) and (np.std(tra.append(coa)) !=0.0) :
    ...
    def compare_categorical(self, return_table=False):
        ...
        for col in self.show_col:
            ...
            if (not is_continuous) and (not (col in self.exclude)):
                if (self.data[col].value_counts().tail(1)!=0).all() and (self.matched_data[col].value_counts().tail(1)!=0).all() :
